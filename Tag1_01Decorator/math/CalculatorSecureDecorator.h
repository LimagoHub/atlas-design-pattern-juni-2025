...
namespace math {
    ...
    class CalculatorSecureDecorator: public Calculator {
        using CalcPointer = std::unique_ptr<math::Calculator>;
        CalcPointer calculator_;

    public:
        explicit CalculatorSecureDecorator(CalcPointer calculator) : calculator_(std::move(calculator)) {}

        double add(double a, double b) override {
            std::cout << "Du kommst hier rein\n";
